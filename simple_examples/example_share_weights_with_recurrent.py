...
def dial_episode(i):
    ...
    msg2, msg1 = None, None 
    h1, h2 = torch.zeros(1, 128) , torch.zeros(1, 128) # GRU cell  inside 
    for step in range(env.max_steps):
        msg1, h1 = msg_net1(obs1, h1) # <- must be recurretn 
        msg2, h2 = msg_net2(obs2, h2) #< - must be recurrent  is not 10 rols and 1 are the same 
        msg1.retain_grad()
        msg2.retain_grad()
        ...
        with torch.no_grad():
            next_msg1,_ = msg_net1(obs1_next,h1)
            next_msg2,_ = msg_net2(obs2_next,h2)
...
msg_dim = 1
msg_net1 = msg_net2 = MessageNetRecuurent(msg_dim=msg_dim)
