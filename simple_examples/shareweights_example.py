...
for game_length in range(1, 5):
    env = GuessMyNumberEnv(batch_size = 1, max_steps = game_length)
    msg_dim = 1
...
for k in all_exp.keys():
    ax.plot(all_exp[k]["loss"], label=f"Loss for game length {k}")
    ax2.plot(all_exp[k]["reward"], label=f"Reward for game length {k}")
